...
const command: Command = {
    ...
    async execute(interaction: ChatInputCommandInteraction) {
        ...
        if (subcommand === 'daily') {
            ...
            if (totalHarvest === 0) {
                return interaction.reply({ content: '⏳ Your assets have not generated enough credits to harvest yet.', ephemeral: true });
            }
            ...
            if (target.id === userId) return interaction.reply({ content: '❌ Self-robbery prohibited.', ephemeral: true });
            if (target.bot) return interaction.reply({ content: '❌ Cannot rob automated systems.', ephemeral: true });
            ...
            if (targetBalance < 200) {
                return interaction.reply({ content: `❌ **${target.username}** has insufficient funds to warrant a heist.`, ephemeral: true });
            }
            ...
            if (balance < amount) return interaction.reply({ content: `❌ Insufficient credits.`, ephemeral: true });

            const win = Math.random() < 0.45;
            const delta = win ? Math.floor(amount * 1.8) - amount : -amount;

            await db.execute('UPDATE users SET balance = MAX(0, balance + ?) WHERE user_id = ?', delta, userId);
            const after = await db.fetchOne('SELECT balance FROM users WHERE user_id = ?', userId);
            ...
            if (balance < SLOTS_COST) return interaction.reply({ content: `❌ Slots cost **${SLOTS_COST} cr**.`, ephemeral: true });
            ...
            if (payout > 0) await db.execute('UPDATE users SET balance = balance + ? WHERE user_id = ?', payout, userId);
            const after = await db.fetchOne('SELECT balance FROM users WHERE user_id = ?', userId);
            ...
            if (target.id === userId) return interaction.reply({ content: '❌ Self-transfers prohibited.', ephemeral: true });
            if (target.bot) return interaction.reply({ content: '❌ Bots cannot hold assets.', ephemeral: true });

            const senderData = await db.fetchOne('SELECT balance FROM users WHERE user_id = ?', userId);
            if (!senderData || senderData.balance < amount) return interaction.reply({ content: `❌ Insufficient funds.`, ephemeral: true });
            ...
            if (!top || top.length === 0) return interaction.reply({ content: '❌ No fiscal data found.', ephemeral: true });
            ...
            if (action === 'deposit') {
                if (liquid < amount) return interaction.reply({ content: `❌ Insufficient liquid funds.`, ephemeral: true });
                ...
                if (vault < amount) return interaction.reply({ content: `❌ Insufficient vault funds.`, ephemeral: true });
